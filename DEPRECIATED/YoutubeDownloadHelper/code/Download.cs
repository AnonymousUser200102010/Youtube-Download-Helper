    ...
    public class Download : IDownload
    {
        ...
        private static ObservableCollection<Video> GetUnfinishedDownloads (ObservableCollection<Video> finishedUrls)
        {
			...
			for (int count = 0, numberOfVideos = finishedUrls.Count; count < numberOfVideos; count++)
			{
				...
				if (finishedUrls.Count <= 0 || !finishedUrls.Any(item => item.Location.Equals(videoInfo.Location)))
				{
        ...
        public void HandleDownloadingProcesses (int retryCount, ObservableCollection<Video> Videos)
        {
                ...
                for (int count = 0, numberOfVideos = Videos.Count; count < numberOfVideos; count++)
                {
                    ...
                    if (MainForm.CurrentlyDownloading)
                    {
                        ...
                        try
                        {
                            ...
                            if (DownloadVideo(video, position) != null)
                            {
								
                                finishedUrls.Add(video);
                        ...
                        catch (Exception ex)
                        {
                            ...
                            if (retryCount <= 3)
                            {
                                ...
                                HandleDownloadingProcesses((retryCount + 1), Videos);
        ...
        private string DownloadVideo (Video video, int position)
        {
            ...
            MainForm.DownloadLabel = string.Format(CultureInfo.InstalledUICulture, "Beginning download from '{0}'", video.Location);
